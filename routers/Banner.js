...
router.post('/', validateImageUpload, upload.array('images', 7), async (req, res) => {
  try {
    const images = req.files.map(file => ({
      url: file.path,
      public_id: file.filename
...
router.put('/:id', validateBannerId, validateImageUpload, upload.array('images', 10), async (req, res) => {
  try {
